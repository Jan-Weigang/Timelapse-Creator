...
set deflickersize=15
set deflickermode=am
set deflicker=size=%deflickersize%:mode=%deflickermode%
set deflickertoggle=on

set proxytoggle=off

set motiontoggle=off
...
echo.
echo.
echo      [90m- - advanced settings: - -[0m
echo      [90mp. Toggle Proxy ([36m%proxytoggle%[90m)[0m
echo      [90md. Toggle Deflicker ([36m%deflickertoggle%[90m)[0m
echo      [90mm. Toggle Motion Interpolation ([36m%motiontoggle%[90m)[0m
echo.
echo      0. GIFs
echo.
echo [30m
...
if %ERRORLEVEL% EQU 6 (
    if %deflickertoggle% EQU on (
	set deflickertoggle=off
	set deflicker=bypass=1
    ) else (
	set deflickertoggle=on
	set deflicker=size=%deflickersize%:mode=%deflickermode%
    )
...
choice /C 123456789pdm0 /N
if %ERRORLEVEL% EQU 13 goto process
...
set deflickername=
if %deflickertoggle% EQU on set deflickername=_deflicker
set motionname=
if %motiontoggle% EQU on set motionname=x2

set /A doublefps=%fps% * 2
set motion=
if %motiontoggle% EQU on set motion=,minterpolate='mi_mode=mci:mc_mode=aobmc:vsbmc=1:fps=%doublefps%:me=ds'
...
if %codec% EQU prores (
	...
	-filter:v "crop=%width%:%height%:exact=1,setsar=1,deflicker=%deflicker%%motion%"^
	"%dirfull%\%name%%codec%_%fps%%motionname%%deflickername%.mov"
)
...
if %proxytoggle% EQU off goto end
...
if %codec% EQU prores (
	...
	-c:v %codec% -profile:v 2 -alpha_bits 0 -loglevel error -stats ^
	-filter:v "crop=1280:720:exact=1,setsar=1,deflicker=%deflicker%"^
	"%dirproxy%\%name%%codec%_%fps%%deflickername%.mov"
)
...
ffmpeg.exe -framerate %fps% -i %name%%%04d.%imagetype% -s %width%x%height% -pix_fmt rgb8 -c:v gif ^
...
ffmpeg.exe -framerate %fps% -i %name%%%04d.%imagetype% -s %width%x%height% -pix_fmt rgb8 -c:v gif -fs %maxsize%M ^
    -loglevel error -stats ^
